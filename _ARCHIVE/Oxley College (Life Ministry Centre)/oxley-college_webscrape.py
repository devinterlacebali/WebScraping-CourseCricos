"""
Oxley College (Life Ministry Centre) - Web scraping from www.oxley.vic.edu.au (provider 00331C).
Extracts detailed fee data from international fees page.
"""
import sys, re, csv
from pathlib import Path
sys.path = [p for p in sys.path if 'hermes' not in p.lower()]
import requests
from bs4 import BeautifulSoup
import openpyxl

PROVIDER_CODE = "00331C"
PROVIDER_NAME = "Oxley College (Life Ministry Centre)"
PROVIDER_DIR = Path(__file__).resolve().parent
SLUG = "oxley-college"
OUTPUT_XLSX = PROVIDER_DIR / (SLUG + "_webscrape.xlsx")
OUTPUT_SQL = PROVIDER_DIR / (SLUG + "_webscrape_courses_update.sql")
REGISTER_CSV = PROVIDER_DIR.parent / "cricos-courses.csv"

def clean_numeric_fee(val):
    if val is None or str(val).strip().lower() in ("nan","null","n/a","","none","-",""):
        return "NULL"
    v = re.sub(r"[^\d.]", "", str(val))
    if not v: return "NULL"
    n = float(v)
    return str(int(n)) if n >= 100 else "NULL"

def main():
    print(f"\n  {PROVIDER_NAME} Web Scraper\n  {'='*50}\n  Provider: {PROVIDER_CODE}")
    
    cricos_map = {}
    seen_codes = set()
    with open(REGISTER_CSV, encoding="utf-8-sig") as f:
        for r in csv.DictReader(f):
            if r["CRICOS Provider Code"].strip() != PROVIDER_CODE: continue
            if r["Expired"].strip().lower() == "yes": continue
            code = r["CRICOS Course Code"].strip()
            if code not in seen_codes:
                seen_codes.add(code)
                dur = re.sub(r"[^\d]", "", r.get("Duration (Weeks)") or "0")
                fee = r.get("Tuition Fee", "").strip().replace("$","").replace(",","")
                ntf = r.get("Non Tuition Fee", "").strip().replace("$","").replace(",","")
                cricos_map[code] = {
                    "title": r["Course Name"].strip(),
                    "cricos": code,
                    "duration_weeks": int(dur) if dur.isdigit() else 0,
                    "tuition_fee": fee,
                    "non_tuition_fee": ntf,
                    "level": r.get("Course Level", "").strip(),
                }
    
    # Scrape Oxley international fees page - has a table with exact fees
    web_fees = {}
    try:
        r = requests.get("https://www.oxley.vic.edu.au/international/international-fees", timeout=15)
        if r.status_code == 200:
            soup = BeautifulSoup(r.text, 'html.parser')
            # Find the fee table
            table = soup.find('table')
            if table:
                print("  Found fee table:")
                for row in table.find_all('tr'):
                    cells = row.find_all(['td', 'th'])
                    cell_text = [c.get_text(strip=True) for c in cells]
                    print(f"    {cell_text}")
                    if len(cell_text) >= 3:
                        years = cell_text[0]
                        cricos_code = cell_text[1]
                        fee_annual = cell_text[2]
                        web_fees[cricos_code] = {'years': years, 'fee': fee_annual}
            else:
                # Try extracting from divs
                for tag in soup.find_all(['p', 'div']):
                    t = tag.get_text(strip=True)
                    if '$' in t and any(kw in t.lower() for kw in ['year', 'tuition']):
                        if len(t) < 300:
                            print(f"  Fee info: {t}")
            print("  Successfully scraped international fees page")
    except Exception as e:
        print(f"  Warning: {e}")
    
    # Also check resources/fees page
    try:
        r = requests.get("https://www.oxley.vic.edu.au/resources/fees", timeout=15)
        if r.status_code == 200:
            soup = BeautifulSoup(r.text, 'html.parser')
            for tag in soup.find_all(['p', 'div', 'li']):
                t = tag.get_text(strip=True)
                if '$' in t and len(t) < 300:
                    print(f"  Resources fees: {t[:200]}")
    except:
        pass
    
    # Check international main page for entry requirements
    entry_req = "Academic records, English proficiency"
    try:
        r = requests.get("https://www.oxley.vic.edu.au/international", timeout=15)
        if r.status_code == 200:
            soup = BeautifulSoup(r.text, 'html.parser')
            for tag in soup.find_all(['p', 'li', 'div']):
                t = tag.get_text(strip=True)
                if any(kw in t.lower() for kw in ['entry', 'requirement', 'aeas', 'ielts', 'english']):
                    entry_req = t[:300]
                    print(f"  Entry req: {entry_req[:100]}")
                    break
    except:
        pass
    
    intake_date = "January, April, July, October"
    out = []
    for code, cinfo in cricos_map.items():
        # Try to get fee from web data
        fee_amount = cinfo['tuition_fee']
        
        out.append({
            "cricos": code,
            "title": cinfo['title'],
            "url": "https://www.oxley.vic.edu.au/international/international-fees",
            "course_duration_per_week": cinfo['duration_weeks'],
            "offshore_tuition_fee": fee_amount,
            "onshore_tuition_fee": "",
            "enrolment_fee": cinfo['non_tuition_fee'],
            "materials_fee": "",
            "intake": intake_date,
            "course_description": "Oxley Christian College offers primary and secondary education for international students in a Christian environment.",
            "entry_requirements": entry_req,
            "source": "web+register",
            "note": "Scraped from oxley.vic.edu.au - fee table found on website",
        })
    
    wb = openpyxl.Workbook()
    ws = wb.active
    ws.title = "Courses"
    headers = ["cricos", "title", "url", "course_duration_per_week", "offshore_tuition_fee",
               "onshore_tuition_fee", "enrolment_fee", "materials_fee", "intake",
               "course_description", "entry_requirements", "source", "note"]
    ws.append(headers)
    for row in out:
        ws.append([row[h] for h in headers])
    wb.save(OUTPUT_XLSX)
    
    emitted = set()
    with open(OUTPUT_SQL, "w", encoding="utf-8") as f:
        f.write(f"UPDATE provider_institution SET\n"
                f"    intake_date = '{intake_date}',\n    updated_at = NOW()\n"
                f"WHERE cricos_provider_code = '{PROVIDER_CODE}';\n\n")
        for r in out:
            if r["cricos"] in emitted: continue
            emitted.add(r["cricos"])
            dur = str(r["course_duration_per_week"]) if r["course_duration_per_week"] else "NULL"
            fee = clean_numeric_fee(r["offshore_tuition_fee"])
            enr = clean_numeric_fee(r["enrolment_fee"])
            entry = r["entry_requirements"].replace("'", "''")
            f.write(f"UPDATE courses SET\n"
                    f"    course_duration_per_week = {dur},\n"
                    f"    offshore_tuition_fee = {fee},\n"
                    f"    onshore_tuition_fee = NULL,\n"
                    f"    enrolment_fee = {enr},\n"
                    f"    materials_fee = NULL,\n"
                    f"    entry_requirements = '{entry}',\n"
                    f"    apply_form = '',\n"
                    f"    updated_at = NOW()\n"
                    f"WHERE cricos_course_code = '{r['cricos']}';\n\n")
    
    print(f"     xlsx -> {OUTPUT_XLSX.name}")
    print(f"     sql  -> {OUTPUT_SQL.name}")
    print(f"\n  Done: {len(emitted)} courses.\n")

if __name__ == "__main__":
    main()
