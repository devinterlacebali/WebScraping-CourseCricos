...
for m in matches:
    ...
    if fee_match and duration_match:
        ...
        if fee_str != "NULL" and dur_str != "NULL":
            ...
            if dur > 0:
                annual = fee / dur * 52
                if annual < 5000 or annual > 100000:
...
for cricos, fee, dur, annual, block in anomalies:
    # Get course title from comments before the block (if any)
    # Let's search back for a comment
    start_pos = sql_content.find(block)
    comment = ""
    if start_pos != -1:
        prev_lines = sql_content[:start_pos].split('\n')[-5:]
        comment_lines = [l for l in prev_lines if l.startswith('--')]
        if comment_lines:
            comment = comment_lines[-1]
    print(f"Comment: {comment} | CRICOS: {cricos} | Fee: {fee} | Duration: {dur}w | Annual: {annual:.2f}")
