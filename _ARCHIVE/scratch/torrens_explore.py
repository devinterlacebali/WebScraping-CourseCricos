#!/usr/bin/env python3
"""Torrens University Australia Limited — www.torrens.edu.au
Exploration: Cloudflare, footer CRICOS, sitemap, CSV coverage, course page SSR.

CRICOS: 03389E | TEQSA: PRV12209
CSV courses: 159
"""
...
async def main():
    ...
    try:
        ...
        with open("scratch/torrens_homepage.html", "w", encoding="utf-8") as f:
            f.write(r.text)
    ...
    print("=" * 60)
    try:
        with open("scratch/torrens_homepage.html", encoding="utf-8") as f:
            html = f.read()
        footer_m = re.search(r"<footer[^>]*>(.*?)</footer>", html, re.DOTALL | re.IGNORECASE)
        if footer_m:
            ft = re.sub(r"<[^>]+>", " ", footer_m.group(1))
            ft = re.sub(r"\s+", " ", ft).strip()
            print(f"  Footer: {ft[:1500]}")
        else:
            print("  No <footer> tag — scanning:")
            for m in re.finditer(r".{0,60}(CRICOS|PRV|TEQSA|03389).{0,60}", html, re.IGNORECASE):
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            for u in course_urls[:15]:
                print(f"  - {u}")
            with open("scratch/torrens_sitemap.xml", "w", encoding="utf-8") as f:
                f.write(r.text)
        else:
            print("  Not found")
    except Exception as e:
    ...
    # ── 4. CSV coverage ────────────────────────────────────────────
    print("\n" + "=" * 60)
    print("4. CSV COVERAGE (provider 03389E)")
    print("=" * 60)
    ...
    print("=" * 60)
    sample = "/courses/nursing/bachelor-of-nursing"
    try:
        ...
        if r.status_code == 200 and len(r.text) > 500:
            print("  SSR: YES")
            html = r.text.lower()
            for kw in ["course overview", "overview", "description", "duration", "fee", "cricos"]:
                if kw in html:
            ...
            if cricos:
                ...
                for m in re.finditer(r".{0,40}(01009).{0,40}", r.text, re.IGNORECASE):
                    print(f"  CRICOS ctx: {m.group().strip()}")
        else:
            print("  SSR: No")
    except Exception as e:
        print(f"  [ERROR] {e}")
    ...
    print("SUMMARY")
    print("=" * 60)
    print("Cloudflare: NO indicators, NO CF headers, NO challenge (clear)")
    print("CRICOS: 03389E in footer")
    print("Sitemap: /sitemap.xml — 1434 total URLs, 177 course pages")
    print("CSV: 159 courses")
    print("SSR: YES — course page ~335KB with CRICOS + overview content")
