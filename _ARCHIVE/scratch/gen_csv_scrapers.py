"""Batch generate CSV-driven scrapers for 7 providers."""
import sys, csv, re
from pathlib import Path
sys.path = [p for p in sys.path if 'hermes' not in p.lower()]

BASE = Path("C:/Users/Dewa(Interlace)/Documents/Interlace Code/WebScraping-CourseCricos")

providers = [
    ("02815M", "Menzies Institute of Technology", "menzies"),
    ("02925E", "ETEA (Education Training & Employment Australia)", "etea"),
    ("02934D", "SCEI (Southern Cross Education Institute)", "scei"),
    ("03010G", "Queensford College", "queensford"),
    ("03386G", "IHNA (Health Careers International)", "ihna"),
    ("03407G", "IHM (Institute of Health & Management)", "ihm"),
    ("03623M", "Strategix Training Group", "strategix"),
]

template = ''''''
"""
{name} — CSV-driven (provider {code}).
Strategy: CSV-driven (site has JS/no SSR).
"""
import sys, re, csv
from pathlib import Path
sys.path = [p for p in sys.path if 'hermes' not in p.lower()]
...
for code, name, slug in providers:
    folder = BASE / name
    folder.mkdir(exist_ok=True)
    fpath = folder / f'{slug}.py'
    content = template.format(code=code, name=name, slug=slug)
    fpath.write_text(content, encoding='utf-8')
    print(f'Created: {fpath.name} ({code})')
print(f'\nAll {len(providers)} scrapers generated.')
