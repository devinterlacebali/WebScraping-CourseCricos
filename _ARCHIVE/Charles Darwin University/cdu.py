"""
CDU (Charles Darwin University) course scraper — template format.

Cookie-based domestic/international toggle.
CDU_STUDENT_TYPE=international to access international data.
Provider: 00300K
"""
import os
import re
...
# --- constants ---------------------------------------------------------------
PROVIDER_CODE = "00300K"
SLUG = "cdu"
DIR = "Charles Darwin University"
...
def get_page(url: str, tries: int = 3):
    for i in range(tries):
        try:
            r = curl_requests.get(url, impersonate='chrome120', timeout=TIMEOUT, cookies=COOKIES)
...
def _months_in_text(text: str):
    ...
    for tok in re.findall(r"[A-Za-z]{3,9}", str(text)):
        ...
        if key in MONTH_ORDER: found.add(key)
...
def _match_cricos(lookup, title):
    ...
    for csv_key, (cc, _, _) in lookup.items():
        ...
        if len(tw) >= 2 and len(cw) >= 2:
            if len(tw & cw) / max(len(tw), len(cw)) >= 0.6: return cc
    return ""
...
def extract_course_description(page):
    heading = page.find("h2", string=re.compile(r"^Overview", re.I))
    if not heading:
        heading = page.find("h2", string=re.compile(r"About", re.I))
    if not heading: return ""
    parts = []
    for sib in heading.find_all_next(["p", "ul", "ol"], limit=8):
        if sib.name == "h2" and sib.get_text(strip=True) and sib.get_text(strip()) not in heading.get_text(strip()):
            break
...
def extract_cricos(page, full_text, csv_lookup, title):
    # CDU format: "CRICOS Code 110624H"
    m = re.search(r"CRICOS\s*(?:Code)?[^\d]*(\d{6,7}[A-Za-z]?)", full_text, re.I)
    if m:
        cc = m.group(1)
        if len(cc) >= 7: return cc
    return _match_cricos(csv_lookup, title)

def extract_fee(page, full_text, csv_lookup, title, duration_weeks):
    # CDU format: "The annual tuition fee ... AUD $32,760"
    # Look for "International tuition fees" then AUD amount
    m = re.search(r"International tuition fees[^$]*AUD\s*\$?\s*([0-9,]+)", full_text, re.I)
    if m:
        ...
        if val > 1000:
            ...
            return str(val)
    # Broader search
    m = re.search(r"tuition[^$]*AUD\s*\$?\s*([0-9,]+)", full_text, re.I)
    if m:
        val = int(m.group(1).replace(",", ""))
        if val > 1000:
            if duration_weeks and str(duration_weeks).isdigit():
...
def extract_entry_requirements(page):
    heading = page.find("h2", string=re.compile(r"Admission requirements", re.I))
    if not heading:
        heading = page.find("h2", string=re.compile(r"Entry requirements", re.I))
    if not heading: return ""
    parts = []
    for sib in heading.find_all_next(["p", "ul", "ol"], limit=12):
        if sib.name == "h2" and sib.get_text(strip=True):
            txt = sib.get_text(strip=True).lower()
            if any(kw in txt for kw in ["fees", "duration", "overview", "career"]):
                break
        if sib.name in ["p", "ul", "ol"] and sib.get_text(strip=True):
            parts.append(str(sib))
    if not parts: return ""
...
def scrape_course(row, csv_lookup):
    ...
    if "404" in title or not title:
        return {"cricos": "", "title": title, "url": url, "course_description": "",
    ...
    return {
        ...
        "offshore_tuition_fee": extract_fee(page, full_text, csv_lookup, title, dur),
        "onshore_tuition_fee": "NULL", "enrolment_fee": "NULL", "materials_fee": "NULL",
...
def build_driver():
    ...
    course_urls = sorted(set(u for u in urls if "/study/course/" in u.lower() and "exchange" not in u.lower()))
    rows = []
...
def main():
    ...
    for i, (_, row) in enumerate(df.iterrows(), 1):
        ...
        results.append(d)
        if d.get("intake"):
            for m in d["intake"].split(", "): all_intakes.append(m)
