"""
The Hamilton and Alexandra College — Web scrape (00144F).
No accessible international fee page. Using CSV data with note.
"""
...
SITE = "https://www.hamiltoncollege.vic.edu.au/international-students"
...
def main():
    ...
    with open(REGISTER_CSV, encoding="utf-8-sig") as f:
        for rec in csv.DictReader(f):
            ...
            csv_total = rec.get("Estimated Total Course Cost","").replace("$","").replace(",","")
            
            rows.append({
                "cricos": rec["CRICOS Course Code"].strip(),
                "course_title": rec["Course Name"].strip(),
                "url": SITE,
                "course_duration_per_week": int(dur) if dur.isdigit() else "",
                "offshore_tuition_fee": "" if clean_fee(fee)=="NULL" else clean_fee(fee),
                "onshore_tuition_fee": "",
                "enrolment_fee": "" if clean_fee(nt)=="NULL" else clean_fee(nt),
                "materials_fee": "",
                "intake": "February, July",
                "course_description": "<h4>Course Overview</h4><p>The Hamilton and Alexandra College - International Program for secondary students.</p>",
                "entry_requirements": "Contact school for international student entry requirements. AEAS testing may apply.",
                "apply_form": "https://www.hamiltoncollege.vic.edu.au/enrolment/",
                "source": "register",
                "note": "No accessible international student fee page. Using CSV data. Contact school for current fees." if not fee else "Fees from CSV register.",
            })
    
    pd.DataFrame(rows).to_excel(OUTPUT_XLSX, index=False)
    emitted = set()
    with open(OUTPUT_SQL, "w", encoding="utf-8") as f:
        f.write(f"-- Update provider institution details\nUPDATE provider_institution SET intake_date = 'February, July', updated_at = NOW() WHERE cricos_provider_code = '{PROVIDER_CODE}';\n\n")
        for r in rows:
            ...
            fee = clean_fee(r["offshore_tuition_fee"])
            enr = clean_fee(r["enrolment_fee"])
